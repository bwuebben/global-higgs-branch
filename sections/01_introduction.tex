...
The geometric theorem has two levels. Under the stated compactness, Hodge and
admissibility hypotheses, \eqref{eq:intro-main} is an exact first-order
statement. A separate vanishing-period argument constrains genuine analytic
arcs to all orders, including arcs tangent to a discriminant to arbitrarily
high order. It implies a full-support relation for nodes, $E_2$ points and
$E_3$ points smoothed through $\cH_{\mathfrak{sl}_2}$. The
two-dimensional component $\cH_{\mathfrak{sl}_3}$ requires a two-period
extension that is not claimed here. Conversely, the general integration of
an allowed first-order
class remains open; it is proved in the explicit positive example below.
...
separates the exact first-order image theorem from the all-orders obstruction.
...
Section~\ref{sec:global-theorems} states the geometric first-order and
all-orders theorems. Section~\ref{sec:examples} presents the absent,
cooperative and obstructed Higgsing examples. Section~\ref{sec:decoupling}
