...
determines the exact image of the moment-map locus, and separates first-order
lifting from all-orders necessity. The positive and negative examples include
...
Several extensions would sharpen the result.

\begin{enumerate}
 \item \emph{Integrability.} The principal mathematical problem is to decide
 whether every full-support class in $\Kbranch$ integrates on the
 non-$\QQ$-factorial locus. A positive answer would give a mixed
 necessary-and-sufficient criterion generalizing the nodal theorem.

 \item \emph{The $E_3$ plane at all orders.} The first-order theorem includes
 $\cH_{\mathfrak{sl}_3}$. The period proof should be extended from one
 distinguished period to a rank-two pairing detecting both $A_2$ directions.

 \item \emph{Controlled rigid limits.} Explicit K\"ahler rays for $X_9$ and
 $X^{\circ}$ would determine whether any of their shared gaugings survives
 after gravity decouples. This would turn the general projection
 $P_{\mathrm{dyn}}Q_X$ into a concrete glued-QFT construction.
 ...
 cohomology would settle the corresponding discrete gauge remnants and line
 operators.
 ...
 \item \emph{Broader sectors.} Non-isolated singularities, higher-rank 5d
 SCFTs and non-toric compact Calabi--Yau threefolds should admit analogous
 maps, but their branch lattices and local Hodge comparisons require new
 input.
\end{enumerate}
