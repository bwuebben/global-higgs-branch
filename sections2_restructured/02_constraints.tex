...
A vanishing-period argument also imposes a necessary relation on
an analytic smoothing, even when its first derivative vanishes:
the branch kernel must project nontrivially to every node,
every $E_2$ point and every line-smoothed $E_3$ point. The precise
statement is given in Appendix~\ref{app:deformations}. It gives
no all-orders conclusion on the two-dimensional $E_3$ component.
Nor does first-order lifting prove general integrability. The
actual $X_9$ smoothing will be supplied independently in
