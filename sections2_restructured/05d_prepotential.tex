...
The eighth-period match also fixes the projective special K\"ahler
geometry of the surviving vectors. In the common normalization,
\begin{equation}
 e^{-K_{\rm vec}}=i\bigl(\overline{X^A}F_A-X^A\overline{F_A}\bigr),
 \qquad (X^A)=(1,\tau_i),\quad (F_A)=(G,F_i),
 \label{eq:x9-d6-special-kahler}
\end{equation}
has the same restriction on both sides, as does its tangential
metric. The terms involving the three vanishing electric coordinates
drop out of the original pairing. Thus the identification includes
the graviphoton period and is stronger than equality of the Yukawa
couplings alone. This is an equality of the reduced vector geometry on the marked branch.
