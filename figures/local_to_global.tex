...
\draw[arrow] (node.south) -- (charge.north west);
\draw[arrow] (e2.south) -- (charge.north);
\draw[arrow] (e3.south) -- (charge.north east);
\draw[arrow] (charge) -- node[above,font=\scriptsize]
  {$\mu_I^{\CC}=0$} (kernel);
...
\node[align=center,font=\scriptsize,below=5mm of charge,text width=0.66\textwidth]
  {The same divisor--curve pairing is the wrapped-M2 electric charge,
   ...
   local-to-global map.};
\end{tikzpicture}
\caption{The compact Higgs-gluing mechanism on a fixed $E_3$ branch profile.
...
triplet moment maps; integration to a finite complex deformation is a separate
question.}
