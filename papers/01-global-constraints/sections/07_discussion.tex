...
scope and in physical content. It applies to the stated exact analytic
germs with a projective crepant resolution, including but not limited to
the admissible toric class, and identifies the relation matrix with electric and flavor gaugings.
...
\begin{enumerate}
 ...
 \item \emph{Broader sectors.} The $E_1$ and $E_4$ dictionaries established
 here include the other locally smoothable non-complete-intersection
 del Pezzo cones. Degrees one through four require a new mixed
 local-to-global comparison: for a cubic cone, for example, the full
 deformation tangent has dimension $16$ but the link has second Betti
 number $6$ \cite[Section~1.2]{Wuebben:2026smoothing}. Local smoothability
 does not replace that missing comparison. Non-isolated singularities
 and higher-rank theories likewise require additional operator and charge data.
\end{enumerate}
