...
Pezzo surfaces of degrees five, six and seven, and over
$\PP^1\times\PP^1$ of degree eight. Our conventions are
\begin{equation}
 \begin{array}{c|c|c}
 \text{surface}&\text{degree}&\text{fixed point}\\\hline
 S_8=\PP^1\times\PP^1&8&E_1\\
 S_7=\operatorname{Bl}_2\PP^2&7&E_2\\
 S_6=\operatorname{Bl}_3\PP^2&6&E_3\\
 S_5=\operatorname{Bl}_4\PP^2&5&E_4
 \end{array}
 \label{eq:intro-del-pezzo}
\end{equation}
Nodes support free hypermultiplets, whereas the four surface contractions
give interacting rank-one sectors. This notation avoids the common
ambiguity between the degree of a del Pezzo surface and the number of blown-up
points.
The other degree-eight surface, $\mathbb F_1$, has no reduced smoothing
branch and is excluded. Degrees one through four lie outside the mixed
global theorem used here; their local complete-intersection smoothability
is a different assertion. The original toric examples involve $E_2$ and
$E_3$, but neither the operator comparison nor the intrinsic global
statement is restricted to those examples.
...
For $E_1$ and $E_4$, the Higgs cones are the minimal nilpotent orbit
closures of $\mathfrak{sl}_2$ and $\mathfrak{sl}_5$. Their torus-invariant
rings give a line and the hyperplane $\sum_{i=1}^5z_i=0$, respectively.
Section~\ref{subsec:e1-e4-sectors} identifies the line with the ruling
difference and the five coordinates integrally with the conic-pencil
pairings of $S_5$. These identifications match both the gauging and the
local smoothing discriminant.

\subsection*{Charge constraints and the local-to-global deformation map}

Let $\wideX\to X$ be a smooth projective crepant resolution and let $D_I$
be compact divisor classes spanning $H^2(\wideX;\QQ)$. At a node, denote
the exceptional curve by $C_a$. At a del Pezzo point, let $R_{p\alpha}$ be a root in the summand of
...
\begin{equation}
 ...
 =\sum_a c_{Ia}t_a+\sum_s m^{(8)}_{Is}u_s
  ...
  +\sum_v\sum_{j=1}^4m^{(5)}_{Ivj}c_{vj},
 \label{eq:intro-moment-map}
\end{equation}
with $H_{p1}L_p=H_{p2}L_p=0$ at each $E_3$ point. Here $u_s$ is the
$E_1$ ruling coordinate and $c_{vj}$ are the $E_4$ simple-root coefficients.
...
reduced $E_2$ lines, the $E_1$ lines and the four-dimensional $E_4$ bases.
Let $\cM_{H,\sigma}^{\mathrm{rigid}}$ denote the product
...
linear complex map $\Qcompact$. For the geometric equality below, $X$ is
connected, normal and projective, with trivial canonical bundle and
$h^1(X,\cO_X)=0$; its singularities are precisely of the stated exact
analytic types. The resolution is small at nodes and standard divisorial
at the cones. These intrinsic hypotheses include the admissible toric
class of Section~\ref{subsec:admissible-class}, where ambient divisors
already span the required rational cohomology. Our principal result is
\begin{equation}
 \boxed{
...
Figure~\ref{fig:local-to-global} summarizes the node/$E_2$/$E_3$ inputs
used in the toric examples and their common compact charge map.
\input{figures/local_to_global}
...
image. Every value of the coordinates on a selected local base has a
...
outside all local discriminants. At a node, an $E_1$ or $E_2$ point, or an $E_3$ line,
...
\end{equation}
At an $E_4$ point, all five diagonal coordinates $z_i$ must be nonzero.
A nonzero vector alone is insufficient in either higher-dimensional base.
Necessity holds for arbitrary
...
criterion holds; nodes and $E_1$, $E_2$, $E_4$ points require no additional projection
