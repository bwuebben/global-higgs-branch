...
lattice. Their one-loop logarithmic thresholds reproduce the
integral conifold monodromies and vanish in the surviving neutral
directions. This fixes the singular terms but not the regular
couplings. An explicit residue-preserving Laurent mutation
...
term. It therefore determines the surviving scalar kinetic metric,
gauge-kinetic and theta matrices, including graviphoton mixing,
on the marked branch.

Two limits have different physical outcomes. Sending the Calabi--Yau
volume to infinity at fixed eleven-dimensional Planck length, while
keeping the local scales bounded, removes the shared flavor gauging.
Decompactifying the auxiliary spacetime circle at fixed five-dimensional
Planck scale instead reaches the contraction with finite bulk kinetic
terms. In the latter limit the Kaluza--Klein gap closes, so the
four-hypermultiplet description has no radius-independent energy range.
Along this limit the compact five-dimensional transition thus retains
the shared gauging, but no effective theory of the four light states is
valid uniformly in the radius.
In both limits the volume-normalized K\"ahler class tends to a face on
which the whole del Pezzo surface has zero volume. That face bounds the
face of the nef cone on which only the two nodal curves and the two
exceptional curves $e_1,e_2$ of the del Pezzo surface have zero area;
near large volume, the K\"ahler parameters of the four-node locus lie in its interior. There
the same four states
are massless hypermultiplets of five-dimensional M-theory, without an
auxiliary circle; the Higgs branch at those vacua is studied
in~\cite{Wuebben:2026finiteplanck}.
