...
All three coefficients are nonzero, so
\eqref{eq:exact-smoothing-criterion} proves the existence of a convergent
smoothing. The reduced selected base is smooth by the integrability
theorem; this is not an assertion about the full nonreduced $E_2$ base.
Independently, an explicit ambient construction gives a smoothing
$\mathcal X\to\Delta$ with smooth general fiber $X_{9,t}$
\cite[Theorem~11.8]{Wuebben:2026smoothing}. The construction uses the
...
\end{equation}
The construction determines more than abstract existence: its general
ambient fiber is toric, with the fan specified in
\cite[Section~11.3]{Wuebben:2026smoothing}. It identifies the smooth
threefold used in the classical and mirror comparisons below.
