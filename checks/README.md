...
- Paper 1 (general constraints): `main.tex` and `sections/`.
- Paper 2 (the compact transition): `main2.tex` and `sections2_restructured/`.
- Paper 3 (supporting technical note): `magnetic_probes.tex` and `sections_probes/`.
...
Run from the repository root. The public distribution provides:

```bash
./verify.sh                # ten Python checks and all three LaTeX builds
./verify.sh --full         # all 38 scripts and all three LaTeX builds
./verify.sh --build-only   # all three LaTeX builds, without computations
```
